...
class BankUser(BaseModel):
    ...
    initial_balance: float = Field(
        ge=10, le=200, description="Initial balance should between 10 to 200 "
    )
...
user_db["1"] = BankUser(
    id="1", user_name="Alice", account_number=66666, initial_balance=99
)
user_db["2"] = BankUser(
    id="2", user_name="jhon", account_number=88888, initial_balance=100
)
...
def transfer_balance():
    ...
    sender = next(
        (user for user in user_db.values() if user.account_number == sender_acc_no),
        None,
    )
    reciever = next(
        (user for user in user_db.values() if user.account_number == reciever_acc_no),
        None,
    )
    if not sender:
...
def getloan():
    ...
    current_account = next(
        (user for user in user_db.values() if user.user_name == username), None
    )
